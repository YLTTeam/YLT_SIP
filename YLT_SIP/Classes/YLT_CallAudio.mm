...
@interface YLT_AudioController : NSObject {
    AudioUnit               _rioUnit;
...
@implementation YLT_AudioController
...
@interface YLT_CallAudio () {
}

@property (nonatomic, strong) YLT_AudioController *audioController;

@end

@implementation YLT_CallAudio

YLT_ShareInstance(YLT_CallAudio);
...
- (BOOL)configureAudio {
    if (!_audioController) {
        _audioController = [[YLT_AudioController alloc] init];
    }
    return YES;
}
